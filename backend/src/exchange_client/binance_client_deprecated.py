...
from backend.src.analyst.utils import get_coin_symbol_name_map
...
class BinanceClient(ExchangeAPIClient):
    ...
    def get_crypto_pair_price(self, pair: str):
        ...
        if response.status_code == 200:  # Checking if the request was successful
            ...
            print(
                f"backend :: {self.name} Exchange API client :: get_crypto_pair_price for pair ({pair}) :: Failed to retrieve current price."
            )
    ...
    def post_buy_order(self, trade_from: str, trade_to: str, from_amount: int):
        ...
        """
        print(
            "backend :: BinanceExchange client :: post_buy_order :: response status ",
            response.status_code,
        )
        print(
            "backend :: BinanceExchange client :: post_buy_order :: response body ",
            response.text,
        )
        if response.status_code == 200:
            try:
                ...
                buy_datetime = str(
                    datetime.utcfromtimestamp(response.json()["transactTime"] // 1000).strftime("%Y-%m-%d %H:%M:%S")
                )  # response.json()['transactTime']  # overwrite Datetime with the Datetime of Buy
    ...
    def post_swap_order(
        self, trade_from: str, trade_to: str, from_amount: int
    ):  # Alternative to BUY order with No fees in Binance
    ...
    def post_sell_order(self, trade_from: str, trade_to: str, quantity: float, sell_order_price: float):
        ...
        print(
            f"backend :: BinanceExchange client :: post_sell_order :: Sell order for {quantity} {trade_from} at {sell_order_price} {trade_to}"
        )
        ...
        response = requests.post(f"{url}?{query_string}", headers=headers)
        print(
            "backend :: BinanceExchange client :: post_sell_order :: response status ",
            response.status_code,
        )
        print(
            "backend :: BinanceExchange client :: post_sell_order :: response body ",
            response.text,
        )
        """
        response body:
