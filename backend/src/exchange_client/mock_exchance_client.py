...
class MockExchangeClient(ExchangeAPIClient):
    ...
    def get_spot_balance(self, quote_asset_pair_price: str | None = None) -> dict[str, Any]:
        ...
        res = {
            "BTC": {"free": 1.24, "locked": 0.0, "price": prices[0]},
            "ETH": {"free": 4.6, "locked": 4.6, "price": prices[1]},
            "ADA": {"free": 1005.9, "locked": 500.0, "price": prices[2]},
            "USDT": {"free": 9300.0, "locked": 0.0, "price": prices[3]},
        }
    ...
    def get_orderbook(self, quote_asset: str, base_asset: str, limit: int) -> list[list[dict[str, float]]] | None:
        ...
        bids = [
            {
                "x": i,
                "y": round(random.uniform(0.1, 5), 4),
                "price": round(mid_price - random.uniform(0, 50), 2),
            }
            for i in range(limit)
        ]
