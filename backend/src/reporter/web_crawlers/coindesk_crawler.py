...
def fetch_coindesk_articles(limit: int = 10):
    articles = []
    page = ""
    page_num = 1
            ...
            try:
                if article.find("a", {"class": "category"}).text == "Markets":
                    ...
                    if len(articles) < limit:
                        articles.append(
                            [
                                article.find("a", {"class": "card-title"}).text,
                                article.find("a", {"class": "card-title"})["href"],
                            ]
                        )
...
def fetch_coindesk_article_body(article_url: str):
    ...
    if req.status_code == 200:
        ...
        for article in bs.select(
            "p:not([class])"
        ):  # The paragraph elements with no class contain the body of the article
            try:
