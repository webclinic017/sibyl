...
class Technician:
    ...
    def api_status_check(api_name: str = "all") -> dict[str, str]:
        ...
        if api_name in ["all", "llms", "openai"]:
            res["openai"] = (
                "Active" if APIEncryptedDatabase.get_api_key_by_name("openai") is not None else "Unavailable"
            )
        if api_name in ["all", "llms", "gemini"]:
            res["gemini"] = (
                "Active" if APIEncryptedDatabase.get_api_key_by_name("gemini") is not None else "Unavailable"
            )
        if api_name in ["all", "llms", "hugging_face"]:
            res["hugging_face"] = (
                "Active" if APIEncryptedDatabase.get_api_key_by_name("hugging_face") is not None else "Unavailable"
            )
        if api_name in ["all", "llms", "anthropic"]:
            res["anthropic"] = (
                "Active" if APIEncryptedDatabase.get_api_key_by_name("anthropic") is not None else "Unavailable"
            )
        ...
        if api_name in ["all", "coinmarketcap"]:
            res["coinmarketcap"] = (
                "Active" if APIEncryptedDatabase.get_api_key_by_name("coinmarketcap") is not None else "Unavailable"
            )
        return res
