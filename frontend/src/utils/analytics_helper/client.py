...
def fetch_symbol_analytics(exchange: str, pair_symbol: str, time_int: str, time_limit: int) -> DataFrame | float | None:
    url = f"{BACKEND_SERVER_ADDRESS}/analyst/symbol/klines/analysis?exchange={exchange.lower().replace(' ', '_')}&symbol={pair_symbol}&interval={time_int}&limit={time_limit!s}"
...
def fetch_price_history(
    ...
    url = f"{BACKEND_SERVER_ADDRESS}/analyst/asset/klines?exchange={exchange.lower().replace(' ', '_')}&symbol={pair_symbol}&interval={time_int}&limit={time_limit!s}"
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
        df = DataFrame()
