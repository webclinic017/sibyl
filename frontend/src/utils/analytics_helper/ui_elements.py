...
from frontend.src.utils.analytics_helper.client import (
    fetch_available_assets,
    fetch_symbol_analytics,
)
from frontend.src.utils.analytics_helper.funcs import invert_dict
from frontend.src.utils.analytics_helper.plots import (
    price_history_correlation_heatmap,
    price_history_plot,
    show_analytics,
    show_line_plot_with_analytics,
)
...
def get_price_analytics_form():
    ...
    with container(border=True):
        ...
        base_asset = selectbox(
            label="Choose Base Asset",
            options=all_assets.keys(),
        ...
        with form(key="forecasting_form", border=False, clear_on_submit=False):
            ...
            with cols[0]:
                ...
                quote_asset = selectbox(
                    label="Choose Quote Asset",
                    options=all_assets[base_asset],
                    index=default_index,
                )
            with cols[1]:
                time_limit = number_input(
                    "Choose Sample Time Limit",
                    value=1000,
                    min_value=100,
                    max_value=10000,
                )
            with cols[2]:
                selectbox(
                    ...
                    options=["Binance"],
                    disabled=True,
                    help="The default exchange is Binance, since it contains the most markets available. Does not require an API key.",
                )

            time_int = select_slider(
                "Choose Time Interval ⏱️",
                [
                    "1 minute",
                    "5 minutes",
                    "15 minutes",
            ...
            if sumbit_button:
                ...
                if plot_type == "Candle Plot":
                    price_history_plot(
                        ...
                        time_int_dict[time_int],
                        time_limit,
                        "Candle Plot",
                    )
                else:
                    # Fetch data
                    df, score = fetch_symbol_analytics(
                        "binance",
                        f"{base_asset}{quote_asset}",
                        time_int_dict[time_int],
                        time_limit,
...
def get_correlation_heatmap_form():
    ...
    with form("Correlation Heatmap"):
        ...
        coins = multiselect(
            label="Choose Assets to Correlate",
            options=base_assets["USDT"],
            max_selections=40,
        )
        cols1 = columns(2)
        with cols1[0]:
