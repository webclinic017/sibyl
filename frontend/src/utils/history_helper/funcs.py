from pandas import DataFrame, Series, isnull, to_datetime, to_numeric
from plotly.graph_objects import Figure, Pie, Scatter
from plotly.graph_objs import Figure
from plotly.subplots import make_subplots
from streamlit import (
    dataframe as st_dataframe,
)
from streamlit import (
    error,
    expander,
...
from frontend.src.utils.history_helper.client import (
...
def get_trading_history_line_plot(trade_df: DataFrame) -> Figure | None:
    ...
    status_symbols = [
        ...
        101,
        117,
    ]  # open circle, triangle-down, diamond, square, star
        ...
        if not isnull(sell_dt):  # 'NaT'
            ...
            fig.add_trace(
                Scatter(
                    x=[sell_dt],
                    y=[
                        price_hist_df[price_hist_df["DateTime"] == sell_dt][price_col].values[0]
                    ],  # add [] around y, if it doesnt work
                    mode="markers",
...
def trade_history_instructions():
    with expander("Trading Table information", expanded=False, icon=":material/contact_support:"):
        ...
        with tab0:
            ...
            info(
                "💡 In order to update the status and get the latest information about an **open order**, click on the update button in the sidebar. This will initiate a function that will fetch the order status from the exchange API."
            )
        with tab1:
            write("TBA")
