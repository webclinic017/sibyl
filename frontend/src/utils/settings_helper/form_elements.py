...
from frontend.db.db_connector import fetch_llm_config, update_fields
from frontend.src.utils.client import check_api_status
from frontend.src.utils.settings_helper.client import (
    get_available_local_models,
    set_mock_exchange_status,
)
...
def exchange_form() -> None:
    with st.container(border=True):
        exchange = st.selectbox(
            ...
            options=["Binance Testnet", "Binance", "Coinbase Sandbox", "Mock Exchange"],
        )
        # MOCK EXCHANGE
        if exchange == "Mock Exchange":
            ...
            if api_conn:
                ...
                if st.button(
                    ...
                    if res:
                        st.success(
                            "Mock Exchange is status is now **Disabled**.",
                            icon=":material/task_alt:",
                        )
                    else:
                        st.error(
                            "Disabling Mock Exchange **Failed**",
                            icon=":material/warning:",
                        )
            else:
                st.warning("Mock Exchange is **Disabled**.", icon=":material/warning:")
                if st.button(
                    "Enable Mock Exchange",
                    ...
                    if res:
                        st.success(
                            "Mock Exchange is status is now **Enabled**.",
                            icon=":material/task_alt:",
                        )
                        st.toast(
                            "Mock Exchange is now **Active**",
                            icon=":material/task_alt:",
                        )
                    else:
                        st.error(
                            "Enabling Mock Exchange **Failed**",
                            icon=":material/warning:",
                        )
        else:
            with st.form("Exchange API Credentials", border=False):
                ...
                if api_conn:
                    ...
                    st.warning(
                        "No active API Key found on the Database, please initialize.",
                        icon=":material/warning:",
                    )
                ...
                if exchange == "Binance":
                    st.info(
                        "In case you have not generated an API key for your Binance Account, see instructions below:",
                        icon=":material/info:",
                    )
                    st.page_link(
                    ...
                    st.info(
                        "In case you have not generated an API key for your Binance Testnet Account, see instructions below:",
                        icon=":material/info:",
                    )
                    st.page_link(
                        "https://www.binance.com/en/support/faq/detail/ab78f9a1b8824cf0a106b4229c76496d",
                        label="Binance Testnet FAQ",
                        icon="🌐",
                    )
                elif exchange == "Coinbase Sandbox":
                    st.page_link(
                        "https://public-sandbox.exchange.coinbase.com/",
                        label="Coinbase Sandbox Website",
                        icon="🌐",
                    )

                with st.expander("API Credentials", expanded=True):
                    exchange_api_key = st.text_input(
                        "API Key",
                        placeholder="Type or Copy/Paste API Key here...",
                        type="password",
                    )
                    exchange_secret_key = st.text_input(
                        "Secret Key",
                        placeholder="Type or Copy/Paste Secret Key here...",
                        type="password",
                    )
                    if exchange == "Coinbase Sandbox":
                        passphrase = st.text_input(
                            "Passphrase",
                            placeholder="Type or Copy/Paste Passphrase here...",
                            type="password",
                        )
                    else:
                        st.text_input(
                ...
                if api_submit:
                    ...
                    if res:
                        st.success(
                            f"✅ {exchange} **API Key** and **Secret Key** have been successfully added/updated to the Encrypted Database."
                        )
                    else:
                        st.error(
                            f"⚠️ Inserting **{exchange} API Key** and **Secret Key** to the Encrypted Database failed."
                        )
    ...
    with st.container(border=True):
        ...
        if llm_type == "API":
            llm_api = st.selectbox(
                ...
                options=["Hugging Face", "OpenAI", "Anthropic", "Gemini"],
                help="Update LLM API",
            )
            ...
            with st.form("API Credentials"):
                st.info(
                    f"In case you have not generated an API key from {llm_api}, see instructions below:",
                    icon=":material/info:",
                )
                page_link = {
            ...
            st.caption(
                "After choosing a Library, an LLM model and it's parameters, it will be automatically downloaded and configured on your local system. Make sure to choose a model that your system can handle based on the RAM and CPU."
            )
            ...
            with st.expander("**3.2 Advanced Options**"):
                ...
                quantization = st.pills(
                    "Quantization",
                    options=["Q2_K", "Q3_K", "Q4_K", "Q5_K", "Q6_K", "Q7_K", "Q8_K"],
                    default="Q4_K",
                    disabled=True,
                )
            ...
            if llm_model_choice:
                if st.button(
                    f"Download and Setup **{llm_model_choice}** LLM Model",
                    type="primary",
...
def price_api_form() -> None:
    with st.form("Crypto Price Credentials"):
        price_history_api = st.selectbox(
            "Choose Price History API",
            options=["CoinCap API", "CoinGecko API", "CoinMarketCap API"],
            disabled=True,
        )
        st.write(
            "The crypto prices are fetched through the Binance API and the CoinCap API (https://docs.coincap.io/). If the limit is reached, please use a custom API key."
        )
...
def backend_form(db_fields: dict) -> None:
    with st.form("Backend Server Settings"):
        serv_ip = st.text_input(
            "Server IP",
            value=db_fields["backend_server_ip"],
            placeholder="Default: 127.0.0.1",
        )
        serv_port = st.text_input(
            "Server Port",
            value=db_fields["backend_server_port"],
        ...
        if back_submit:
            update_fields(
                backend_server_ip=serv_ip,
                backend_server_port=serv_port,
                backend_server_secure=0,
            )  # Update NLP Model Choice in frontend SQlite3 DB
            st.success(
                f"Server Parameters Update Successfully, New Configurations: [{current_backend_address}] -> [{new_backend_address}]"
            )
...
def oracle_form() -> None:
    with st.container(border=True):
        st.write(
            "You fist have to setup a **Local** or **API** LLM model from the *LLM Settings tab* 👈. After that you have to choose an **LLM model** for the **Oracle Engine**."
        )
        ...
        if llm_info:
            ...
            st.warning(
                "**Oracle Engine** is **not configured**. Please select an LLM Model.",
                icon=":material/warning:",
            )
        st.divider()
        ...
        if "model_name" in locals() and model_name:
            if st.button("Set new default Oracle Model", type="primary", icon=":material/cached:"):
                try:
                    update_fields(
                        llm_source=model_source.lower(),
                        llm_type=model_type.lower().replace(" ", "_"),
                        llm_name=model_name,
                    )
                    st.success(
                        "Oracle Model has been set successfully and is ready to go. Refresh page to see the changes.",
                        icon=":material/check:",
                    )
