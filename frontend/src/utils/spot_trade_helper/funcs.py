...
from frontend.src.utils.spot_trade_helper.client import (
    fetch_asset_market_price,
    post_spot_trade,
)
...
def get_account_balance(quote_asset: str, quantity: float, market_price: float) -> None:
    ...
    if account_balance_key not in st.session_state:  # if account balance calculated
        with st.spinner("Fetching account balance..."):
            ...
            st.session_state[account_balance_key] = {
                asset: price["free"] for asset, price in account_balance["spot_balances"].items()
            }
    ...
    if quote_asset in st.session_state[account_balance_key].keys():
        if market_price is not None:
            ...
            st.info(
                f"Your account balance is **{st.session_state[account_balance_key][quote_asset]} {quote_asset}**, don't choose a quantity larger than your balance because the **order will fail**.",
                icon=":material/counter_5:",
            )
    else:
        st.error(
            f"Warning: No available **{quote_asset}** in your account. This order will fail.",
            icon=":material/counter_5:",
        )
...
def get_pair_market_price(quote_asset: str, base_asset: str, quantity: float, min_trade_value: float) -> int | None:
    if (
        st.session_state["trade_exchange_api"] == "Coinbase"
        or st.session_state["trade_exchange_api"] == "Coinbase Sandbox"
    ):
    ...
    if market_price:
        st.info(
            f"**{base_asset} Current price**: {market_price} {quote_asset}",
            icon=":material/counter_2:",
        )
        st.info(
            f"You'll trade **{round(quantity * market_price, 4)} {quote_asset}** for **{quantity} {base_asset}**",
        ...
        if quantity * market_price >= min_trade_value:
            st.success(
                "The **minimum trade value limit** of **"
                + str(min_trade_value)
                + "** for the "
                + trading_pair
                + " pair is satisfied!",
        ...
        st.warning(
            "**Failed** to fetch current trading pair **market price**.",
            icon=":material/counter_2:",
        )
        st.warning(
            "**Failed** to calculate the **quantity** of **Quote asset** necessary for this order..",
            icon=":material/counter_3:",
        )
        st.warning(
            f"**Failed** the **minimum trade quote limit** for this order is {min_trade_value} cannot calculate feasibility based on market price..",
            icon=":material/counter_4:",
        )
...
def submit_order(
    ...
    with st.spinner("Sending Test Order..."):
        test_res = post_spot_trade(
            True,
            st.session_state["trade_exchange_api"],
            order_type,
            quote_asset,
            base_asset,
            side,
    ...
    if test_res is None or "status" not in test_res.keys():
        ...
        if test_res["status"] == "success":
            st.success(
                "Test Order was **validated**, your Trade Order being placed right now...",
            ...
            with st.spinner("Placing Order..."):
                ...
                if res is None or "status" not in res.keys():
                    ...
                    if res["status"] == "success":
                        ...
                        st.link_button(
                            "You can find the SPOT order in the Trading History tab.",
                            "http://localhost:8501/trading_report ",
                            type="secondary",
                            icon=":material/youtube_searched_for:",
                        )
                        st.write(
                            f"Below is the response message returned by the {st.session_state['trade_exchange_api']} API."
                        )
                        st.json(res["message"])
                        st.toast("Your Trade Order has been placed successfully.", icon="✅")
                    else:
                        st.warning(
                            f"Order is **invalid**. Error message {res['message']}",
                            icon=":material/warning:",
                        )
        else:
            st.error(
                f"Test Order is **invalid**. Error message {test_res['message']}",
                icon=":material/warning:",
            )
