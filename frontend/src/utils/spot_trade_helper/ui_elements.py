    ...
    with expander("📖 Trading Instructions", expanded=exp):
        ...
        if str_val == 0:
            with container(border=False):
                ...
                with tab0:
                    write(
                        "1. Choose a ***Quote Asset***: This is the asset which you have to have in the account and will be use to **Buy** the **Base Asset**"
                    )
                    ...
                    write(
                        "3. Choose a ***Quantity***: This indicates how much of ***Base Asset*** (e.g. BTC) you will buy."
                    )
                    write(
                        "**Example**: To buy 1 ETH with USDT you have on your account, you will choose: -Quote Asset: USDT, - Base Asset: ETH, quantity: 1."
                    )
                with tab1:
                    write(
                        "This option is similar to the standard order, the only difference is you specify the amount of the Quote Asset you want to spend."
                    )
                    write(
                        "**Example**: You want to buy 10 USDT worth of ETH. You will choose: -Quote Asset: USDT, - Base Asset: ETH, quote_quantity: 10. This way you define how much you want to spend and not how much you want to buy."
                    )
                    info(
                        "**Note**: This option is only available in Market orders.",
                        icon=":material/info:",
                    )
        ...
        if str_val == 1:
            with container(border=False):
                ...
                write("""**Order Type**""")
                write(
                    """1. **Market Order** – Executes immediately at the current market price. Suitable for quick trades but may incur higher "taker" fees."""
                )
                write(
                    """2. **Limit Order** – Allows you to set a specific price at which you want to buy or sell. This order is placed on the order book and may incur lower "maker" fees."""
                )
                write(
                    """3. **Stop-Loss Order** – Triggers a market sell order when the asset price falls to a specified "stop price." Used to minimize losses."""
                )
                write(
                    """4. **Stop-Loss Limit Order** – Similar to a stop-loss, but instead of triggering a market order, it places a limit order once the stop price is reached."""
                )
                write(
                    """5. **Take-Profit Order** – A market order that executes when the asset reaches a specified price, allowing you to secure profits automatically."""
                )
                write(
                    """6. **Take-Profit Limit Order** – Similar to a take-profit order but executes as a limit order at a defined price."""
                )
                write(
                    """7. **Trailing Stop Order** – A stop order that moves dynamically with price changes, protecting profits while minimizing downside risk."""
                )
                write(
                    """8. **OCO (One-Cancels-the-Other)** – Combines a limit order and a stop-loss order, where if one order is executed, the other is automatically canceled."""
                )
                divider()
                write("""**Order Side**""")
                ...
                write(
                    """**Percentage Mode** – If enabled, Stop-Loss and Take-Profit levels will be calculated as percentages rather than absolute values."""
                )
                write(
                    """**Post-Only Order** – Ensures your limit order adds liquidity to the market by preventing it from executing immediately."""
                )
                write("""Time in Force (TIF) Options""")
                write(
                    """- **GTC (Good-Til-Canceled)** – The order stays open until fully executed or manually canceled."""
                )
                write("""- **IOC (Immediate-Or-Cancel)** – The order is executed immediately for available liquidity, and any remaining portion is canceled.
                    FOK""")
                write(
                    """- **FOK (Fill-Or-Kill)** – The order must be fully executed immediately or canceled entirely."""
                )
                write(
                    """**Iceberg Quantity**: If entering a large order, an iceberg order allows only a portion of the total order to be visible on the order book at a time, helping to prevent large trades from impacting the market significantly."""
                )
        if str_val == 2:
            with container(border=False):
                ...
                caption(
                    "Some Exchanges offer the **Token Swap** functionality, which directly swaps your Quote asset to the Base asset in market price with 0 fees."
                )
                write(
                    "1. The **Binance Convert** API enables trading with **0 fees**. If the backend server doesn't find a valid Convert API, the standard order will be used. If you are eligible to use the Convert functionality, the corresponding option will be shown below."
                )
                write(
                    "2. **Coinbase One** which provides zero trading fees among other benefits for a monthly fee of $29.99. This service allows users to buy, sell, and swap cryptocurrencies without incurring individual transaction fees, effectively enabling cost-effective crypto swaps."
                )
                write(
                    "3. Kraken has introduced a feature known as **Swaps** within the **Kraken Wallet**, therefore it is not enabled on the Kraken Exchange."
                )
        ...
        if str_val == 3:
            with container(border=False):
                ...
                write(
                    "First, a **test order** will be created to test if the order is possible based on the trade parameters. If it is not a message with the error from the Exchange API will be shown, otherwise the Trade will be placed."
                )
                ...
                link_button(
                    "Trading Report",
                    "http://localhost:8501/trading_report ",
                    type="primary",
...
def plot_orderbook(exchange: str, quote_asset: str, base_asset: str, limit: int):
    ...
    if data:
        ...
        warning(
            f"Failed to fetch orderbook for {quote_asset}/{base_asset}",
