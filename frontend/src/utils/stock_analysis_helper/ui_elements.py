...
from frontend.src.utils.stock_analysis_helper.client import (
    fetch_stock_advice,
    fetch_stock_details,
)
from frontend.src.utils.stock_analysis_helper.plots import (
    linear_gauge_chart,
    risk_gauge,
)
...
def display_company_info(info: dict, stock_symbol: str):
    ...
    with c0:
        st.write(f":material/id_card: Stock symbol: {stock_symbol}")
    ...
    c0, c1, c2 = st.columns(3)
    with c0:
        st.metric("Current Price", str(info.get("currentPrice")) + "$") if "currentPrice" in info else st.metric(
            "Current Price", "N/A", border=True
        )
    with c1:
        st.metric("Market Cap", f"{info.get('marketCap'):,}$") if "marketCap" in info else st.metric(
            "Market Cap", "N/A"
        )
    with c2:
        st.metric("Enterprise Value", f"{info.get('enterpriseValue'):,}$") if "enterpriseValue" in info else st.metric(
    ...
    st.write(
        f"{info.get('numberOfAnalystOpinions', 'N/A')} **Analysts** generated the following recommendation **{info.get('recommendationKey', 'N/A')}** with a score of **{info.get('recommendationMean', 'N/A')}**."
    )
...
def get_oracle_advice(symbol: str, model_source: str, model_type: str, model_name: str | None = None):
    ...
    if llm_advice is None:
        st.error(
            "Failed to generate advice. Please check logs to see if what went wrong with the **Oracle**.",
            icon=":material/error:",
        )
    else:
    ...
    if stock_details:
        ...
        if oracle_status:
            ...
            if st.button("", type="tertiary"):
                get_oracle_advice(
                    symbol,
                    oracle_status["llm_source"],
                    oracle_status["llm_type"],
                    oracle_status["llm_name"],
                )
            st.caption("**Oracle** is *active*, you can get an stock advice by pressing the Oracle button.")
        else:
            st.info(
        ...
        st.error(
            "Failed to fetch Stock information. Please check logs to see if **YahooFinance API** reached the limit or needs an **upgrade** and try later.",
            icon=":material/error:",
        )
