...
def strategy_params_form(strategy: str) -> dict[str, Any]:
    ...
    if strategy == "Bollinger Bands":
        caption(
            "Bollinger Bands Strategy – Uses Bollinger Bands to identify price volatility. "
        ...
        caption(
            "Bollinger RSI Volume Surge Strategy – A **multi-factor** approach combining Bollinger Bands, RSI, EMA crossover, "
        ...
        c00, c01 = columns(2)
        with c00:
            bb_window = number_input(
                "Bollinger Bands moving average window",
                min_value=1,
                value=20,
                max_value=1000,
            )
        with c01:
            bb_std_dev = number_input(
        ...
        volume_factor = number_input(
            "Multiplier to detect volume spikes",
            min_value=1.0,
            value=1.5,
            max_value=50.0,
        )
        ...
        caption(
            "EMA Crossover Strategy – Tracks **price trends** using two **Exponential Moving Averages** (EMA). "
        ...
        caption(
            "The Impulse Breakout Strategy focuses on detecting **sudden price movements** and **breakouts** from established "
        ...
        with c01:
            bb_std_dev = number_input(
                "Bollinger Bands: Standard Deviations for upper & lower bands",
                min_value=0.1,
                value=2.0,
                max_value=10.0,
        ...
        caption("**RSI**: Relative Strength Index parameters.")
        rsi_window = number_input(
            "RSI: Number of Periods",
            min_value=1,
            value=14,
            max_value=100,
        ...
        with c20:
            macd_short = number_input(
                "MACD: Short-term exponential moving average (EMA) period",
                min_value=1,
                value=12,
                max_value=100,
        ...
        with c21:
            macd_long = number_input(
                "MACD: Long-term exponential moving average (EMA) period",
                min_value=2,
                value=26,
                max_value=100,
        ...
        caption(
            "The Quantum Momentum Strategy uses a combination of** momentum indicators** and **price trends** to identify powerful market moves. "
        ...
        atr_window = number_input(
            "ATR (Average True Range): Window Size",
            min_value=1,
            value=14,
            max_value=100,
        ...
        cmf_window = number_input(
            "CMF (Chaikin Money Flow): Window Size",
            min_value=1,
            value=20,
            max_value=100,
        ...
        c10, c11 = columns(2)
        with c10:
            tsi_long = number_input(
                "TSI (True Strength Index): Long Period",
                min_value=2,
                value=25,
                max_value=200,
        ...
        with c11:
            tsi_short = number_input(
                "TSI (True Strength Index): Short Period",
                min_value=1,
                value=13,
                max_value=200,
        ...
        caption(
            "RSI Strategy – Based on the Relative Strength Index (RSI), this strategy **buys** when the RSI indicates "
    ...
    button(
        "Strategy Parameters LLM Advisor",
...
def get_market_condition_message(score: float) -> None:
    ...
    if score < 30:
        message, color = (
            "❌ Unfavorable Market – High uncertainty, not ideal for trading.",
            "#ff4d4d",
        )
    elif score < 60:
        message, color = (
            "⚠️ Neutral Market – Some opportunities, but risk is present.",
            "#ffcc00",
        )
    elif score < 80:
        message, color = (
            "✅ Good Market – Favorable conditions, likely trends.",
            "#4CAF50",
        )
    else:
        message, color = (
            "🚀 Strong Market – Ideal for trading, strong trend confirmation.",
            "#008CBA",
        )
