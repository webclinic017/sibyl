...
from frontend.src.utils.spot_trade_helper.funcs import (
    get_account_balance,
    get_pair_market_price,
    submit_order,
)
from frontend.src.utils.spot_trade_helper.ui_elements import (
...
if len(st.session_state["available_exchange_apis"]) > 0:
    ...
    st.session_state["trade_exchange_api"] = st.selectbox(
        "Choose Exchange", options=st.session_state["available_exchange_apis"]
    )
    ...
    with st.container(border=False):
        ...
        with col00:
            default_quote_index = (
                list(asset_list.keys()).index("USDT") if "USDT" in asset_list.keys() else 0
        ...
        with col01:
            default_base_index = (
                asset_list[quote_asset].index("BTC") if "BTC" in asset_list[quote_asset] else 0
            )  # making BTC appear as preselected choice
        ...
        with col02:
            ...
            symbol_info = fetch_symbol_info(
                exchange=st.session_state["trade_exchange_api"],
            ...
            quantity = st.number_input(
                "Quantity (Base Asset):",
                min_value=symbol_info["base_precision"],
                step=symbol_info["base_precision"],
                format=base_format,
            )
            st.caption(
                f"The precision is fetched by the {st.session_state['trade_exchange_api']} API to match the minimum accepted Base asset precision for Trading."
            )
    ...
    with st.container(border=False):
        ...
        with col10[0]:
            ...
            st.caption(
                "Select between a Limit or Market order. Limit orders are added to the order book, often with lower 'maker' fees, while Market orders are matched instantly with existing orders, incurring 'taker' fees."
            )
        ...
        with col20:
            if order_type in ["Limit", "Stop-Loss Limit", "Take-Profit Limit"]:
                price = st.number_input(
                    "Limit Price:",
                    min_value=symbol_info["quote_precision"],
                    step=symbol_info["quote_precision"],
                    format=base_format,
                )
            else:
                st.number_input(
        ...
        with col30[0]:
            st.caption(
                "If percentage is enable, the **Stop-Loss** and **Take-Profit** will be based on percentages and not the actual value of the Base Asset."
            )
    ...
    with st.container(border=False):
        ...
        with col10[0]:
            st.caption(
                "The Swap (Binance Convert API) enables trading with 0 fees. If not available, choose Trade option. Read the instructions at the top of the page to minimize the fees."
            )
            with st.spinner("Checking Swap Availability..."):
                ...
                if "success" in swap_status:
                    ...
                    st.session_state["order_type"] = st.segmented_control(
                        "Choose Buy/Sell Order Type",
                        options=["Trade", "Swap"],
                        default="Trade",
                        disabled=True,
                    )
