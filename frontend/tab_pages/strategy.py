...
from frontend.src.utils.strategy_helper.launcher_helper import (
    backtest_evaluation_results,
    get_market_condition_message,
    get_strategy_instructions,
    market_condition_explanation,
    strategy_params_form,
)
from frontend.src.utils.ui_elements import col_style2, fix_page_layout, set_page_title
...
if st.session_state["available_exchange_apis"]:
    oracle_button(module="strategy", enabled=False)
    st.divider()
    with st.container(border=False):
        ...
        with c0:
            ...
            st.caption(
                "Choose the **Quote Asset** which will be used in this strategy. Below you'll see all the assets you have available in your account and the corresponding markets you can deploy the strategy."
            )
            default_quote_index = (
                list(balance.keys()).index("USDT") if "USDT" in balance.keys() else 0
            )  # making USDT appear as preselected choice
        ...
        with c1:
            ...
            st.caption(
                f"Choose the Quote amount you want to trade on this strategy. Each order sent to the {exchange} API will define the quote_quantity and not the base."
            )
            st.warning(
                "**Warning**: Choose only what you can afford to lose, as the whole Quote amount is at risk.",
        ...
        with c2:
            ...
            st.caption(
                f"Choose the Base Asset. The available base assets correspond to the selected quote asset and the available markets at {exchange}."
            )
        ...
        with c3:
            ...
            st.caption(
                "The strategy runs in a loop on a steady time interval. Based on the selected interval, the algorithm will focus on the short-term (**High-Frequency Trading**) or long-term (**Swing Trading**)."
            )
            time_interval = st.pills(
                "Time Interval",
                options=[
                    "1s",
                    "15s",
        ...
        with c5[0]:
            ...
            st.caption(
                "**(7.1)** Number of trades (**pair of BUY-SELL orders**) before the algorithm stops. The larger this value, the more the algorithm will run for."
            )
            trades_num = st.slider("Number of trades", min_value=1, max_value=10, value=2)
            st.caption(
                "**(7.2)** The **Kline dataset size** indicates the size of the dataset that the strategy algorithm uses as input. "
            ...
            with c6:
                st.number_input(
                    "Maximum Execution Price **Slippage** Percentage (%)",
                    min_value=0.0,
                    max_value=100.0,
                    value=1.0,
                    disabled=True,
        ...
        st.divider()
        st.caption(
            "The **Backtesting** will evaluate the effectiveness of the selected trading strategy by running it against historical data to see how it would perform. "
        ...
        if valid_request_flag:
            ...
            if time_interval == "15s":
                st.warning(
                    "Backtesting is not currently available for the **15 second interval**",
                    icon=":material/warning:",
                )
            else:
                if st.button(
                    "Run Backtesting",
                    key="backtesting_button",
                    type="secondary",
                    ...
                    if res is not None:
                        st.divider()
                        st.html(
                            "<h3 style='text-align: left;margin-top:0.1em; margin-bottom:0.1em; padding:0;color:#5E5E5E'>Backtesting Results</h3>"
                        )
                        st.caption(
                            "Currently Backtesting doesn't support **Simulated Order Execution**, **Slippage simulation**, **Position & Risk management** and  **Trading Fees** (~0.1%)"
                        )
                        st.html(
                            "<h4 style='text-align: left;margin-top:0.1em; margin-bottom:0.1em; padding:0;color:#5E5E5E'>1. Backtesting Evaluation Metrics</h4>"
                        )
                        if len(res["metrics"]) > 0:
                            backtest_evaluation_results(res["metrics"])
                        else:
                            st.warning(
                                "No metrics available since no BUY or SELL orders were made by the strategy.",
                                icon=":material/troubleshoot:",
                            )
                        st.html(
                            "<h4 style='text-align: left;margin-top:0.1em; margin-bottom:0.1em; padding:0;color:#5E5E5E'>2. Backtesting Market Condition Score</h4>"
                        )
                        st.caption(
                            "The Backtester class calculates a **score (%)** based on the OHLCV data, mostly relying on the price movement, that indicates whether the market is good for trading or not. **Higher is better**."
                        )
                        market_condition_explanation()
                        get_market_condition_message(res["score"])
                        st.html(
                            "<h4 style='text-align: left;margin-top:0.1em; margin-bottom:0.1em; padding:0;color:#5E5E5E'>3. Strategy Action Plot</h4>"
                        )
                        st.caption(
                            "The algorithm might have decided more BUY, SELL order in between that are invalid due to the fact that two or more consecutive BUY or SELL orders are not allowed."
                        )
                        static_strategy_plot(pd.DataFrame(res["logs"]), False, False)
                        st.write("Deploy Strategy below 👇👇")
                    else:
                        st.error(
                            "Failed to get Backtesting results. See logs for error.",
            ...
            if st.button(
                ...
                if res is not None:
                    st.success(
                        "Strategy has been successfully deployed.",
                        icon=":material/rocket_launch:",
                    )
                    st.toast("Strategy has been successfully deployed.", icon="🚀")
                    st.link_button(
                        "Navigate to the **Strategy Console** to view the progress",
                        "http://localhost:8501/strategy_console",
                        icon=":material/browse_activity:",
                    )
                else:
                    st.error(
                        "Failed to deploy strategy. See logs for error.",
                        icon=":material/report:",
                    )
        else:
            st.button(
