...
from frontend.src.utils.strategy_helper.console_helper import (
    real_time_strategy_plot,
    show_active_strategy_count,
    show_evaluation_metrics,
    static_strategy_plot,
    strategy_info_card,
    strategy_plot_info,
)
...
if strategies:
    ...
    if df_to_show.shape[0] == 0:
        ...
        strategy_info_card(
            ...
            quote_asset=df_to_show["quote_asset"].iloc[0],
            base_asset=df_to_show["base_asset"].iloc[0],
            balance=df_to_show["quote_amount"].iloc[0],
            time_interval=df_to_show["time_interval"].iloc[0],
            trades_limit=df_to_show["trades_limit"].iloc[0],
            strategy_name=df_to_show["strategy_name"].iloc[0],
            created_at=df_to_show["created_at"].iloc[0].strftime("%Y-%m-%d %H:%M:%S"),
            status=df_to_show["status"].iloc[0],
        )

        st.caption(
            "You can **Stop** or **Pause** an active strategy and **download** the **logs in csv** for a strategy that has finished in the sidebar 👈."
        )
        ...
        if logs is not None and len(logs) > 0:
            ...
            if st.sidebar.button(
                ...
                disabled=status_change_options,
                use_container_width=True,
            ):
            ...
            st.caption(
                "The strategy logs ***order*** field contains the action the algorithm too at a specific timestamp. "
            ...
            st.caption(
                "The **Slippage** is how far of the **order price** was from the **desired price** that the algorithm had as the latest price when it made the decision. "
            ...
            if hide_invalid:
                st.dataframe(
                    logs_df[logs_df["order"].isin(["BUY", "SELL"])],
                    use_container_width=True,
                    hide_index=True,
                )
            else:
            ...
            st.html(
                "<h3 style='text-align: left;margin-top:0.1em; margin-bottom:0.1em; padding:0;color:#5E5E5E'>2. Strategy Evaluation Metrics</h3>"
            )
            ...
            plot_options = st.pills(
                "Plot Options",
                ["Show Slippage", "Show Invalid Orders"],
            ...
            if real_time_option:
                real_time_strategy_plot(
                    logs_df[["timestamp", "price", "order"]],
                    strategy_id,
                    df_to_show["time_interval"].iloc[0],
                    show_invalid,
                )
            else:
                static_strategy_plot(
                    logs_df[["timestamp", "price", "slippage", "order"]],
                    show_invalid,
                    show_slippage,
                )
