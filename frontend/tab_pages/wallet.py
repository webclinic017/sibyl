...
from frontend.src.utils.wallet_helper.ui_elements import (
    get_account_information,
    get_logo_header,
    get_pie_chart,
    get_spot_balance_wallet_table,
...
st.caption(
    "Navigate to the **Settings Tab** ⚙️ to add your connect your **Exchange API credentials**. You can also provide your **LLM API keys** or choose an automated **local LLM** deployment, in order to activate the **LLM-powered functionalities**  of the **Oracle** 🔥 using the button on the bottom right. All the API keys are stored in an **encrypted Database** on your local system, with an encryption key generated locally."
)
...
if (
    st.session_state["backend_status"] == "Active"
):  # if connection with backend is on, fetch wallet information, check exchange API Key
    if len(st.session_state["available_exchange_apis"]) <= 0:
        ...
        st.html(
            """<div style="text-align: left; color: #5E5E5E; font-weight: bold; font-size: 18px;"><br>Account Balance<br></div>"""
        )
        st.caption(
            "You can choose a Quote asset below and get the value of each Base asset in your wallet in the Quote Asset."
        )
